...
Xmid = cellfun(@(x) x(:,x0-10:x0+10), I3,'UniformOutput',false);
...
x_crests = cellfun(@(x) x(:,2), centroids,'UniformOutput',false); %The x-position of the crests
x_crests_lhs = cellfun(@(x) x(x<y0-r_min), x_crests, 'UniformOutput',false); %Make sure to not keep too small radii´s. And sort the radii in ascending order
x_crests_rhs = cellfun(@(x) x(x>y0+r_min), x_crests, 'UniformOutput',false); %Make sure to not keep too small radii´s. And sort the radii in ascending order

r_crests = cellfun(@(x) abs(x(:,1)-y0), centroids,'UniformOutput',false); %The radial distance from origin to crest
...
r_crests_lhs = cellfun(@(x) abs(x-y0), x_crests_lhs, 'UniformOutput',false); %Make sure to not keep too small radii´s. And sort the radii in ascending order
r_crests_rhs = cellfun(@(x) abs(x-y0), x_crests_rhs, 'UniformOutput',false); %Make sure to not keep too small radii´s. And sort the radii in ascending order
